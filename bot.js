...
// спільна логіка акційного банера (працює і для фото, і для файлу-документа)
async function handleBanner(chatId, fileId, caption) {
  const cap = (caption || "").trim();
  if (!/акці|akci|sale|знижк/i.test(cap)) {
  ...
  const desc = cap.replace(/^\/?(акція|akcia|sale)/i, "").trim() || cap;
  ...
  try {
    ...
    try {
      ...
      bannerBuffer = Buffer.from(arr);
    } catch (e) { console.error("банер:", e.message); }
    pending[key] = { post: salePost, images: [], bannerBuffer };
...
// банер як ФОТО (стиснуте)
bot.on("photo", async (msg) => {
  await handleBanner(msg.chat.id, msg.photo[msg.photo.length - 1].file_id, msg.caption);
});

// банер як ФАЙЛ-документ (зображення без стиснення)
bot.on("document", async (msg) => {
  const mime = msg.document?.mime_type || "";
  if (!mime.startsWith("image/")) {
    return bot.sendMessage(msg.chat.id, "Це не зображення. Скинь банер як фото або файл-картинку з підписом «акція ...».");
  }
  await handleBanner(msg.chat.id, msg.document.file_id, msg.caption);
});
...
bot.on("callback_query", async (q) => {
  ...
  for (const t of targets) {
    try {
      let sent;
      if (item.bannerBuffer) {
        // акційний пост з банером (буфер — працює для будь-якого бота)
        sent = await posterBot.sendPhoto(t.id, item.bannerBuffer, { caption: item.post }, {
          filename: "banner.jpg", contentType: "image/jpeg",
        });
      } else {
  ...
  if (pool && !item.bannerBuffer && item.title) {
    try {
      await pool.query(
