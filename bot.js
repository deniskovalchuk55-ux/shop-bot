  ...
  try {
    ...
    if (res.ok) {
      ...
      if (text.length >= 500) {
        ...
        let im;
        while ((im = imgTag.exec(html)) !== null) {
          let u = im[1];
...
// надсилає пост: альбомом (кілька фото) або одним фото, або текстом
async function sendPost(target, images, caption) {
  const imgs = (images || []).filter(Boolean).slice(0, 4);
  if (imgs.length >= 2) {
    // альбом: підпис на першому фото
    const media = imgs.map((u, i) => ({
      type: "photo",
      media: u,
      ...(i === 0 ? { caption } : {}),
    }));
    const sent = await target.fn(target.id, media);
    // sendMediaGroup повертає масив повідомлень — беремо перше
    return Array.isArray(sent) ? sent[0] : sent;
  } else if (imgs.length === 1) {
    return await bot.sendPhoto(target.id, imgs[0], { caption });
  } else {
    return await bot.sendMessage(target.id, caption);
  }
}
...
bot.on("message", async (msg) => {
  ...
  try {
    ...
    try {
      await sendPost({ id: chatId, fn: (id, media) => bot.sendMediaGroup(id, media) }, page.images, finalPost);
    } catch (e) {
  ...
  for (const t of targets) {
    try {
      const sent = await sendPost(
        { id: t.id, fn: (id, media) => bot.sendMediaGroup(id, media) },
        item.images,
        item.post
      );
