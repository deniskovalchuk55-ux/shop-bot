...
bot.on("photo", async (msg) => {
  ...
  try {
    ...
    // отримуємо пряме посилання на банер (працює для будь-якого бота)
    let bannerUrl = null;
    try { bannerUrl = await bot.getFileLink(fileId); } catch {}
    pending[key] = { post: salePost, images: [], bannerUrl };
...
bot.on("callback_query", async (q) => {
  ...
  for (const t of targets) {
    try {
      let sent;
      if (item.bannerUrl) {
        // акційний пост з банером (по URL — працює для будь-якого бота)
        sent = await posterBot.sendPhoto(t.id, item.bannerUrl, { caption: item.post });
      } else {
  ...
  if (pool && !item.bannerUrl && item.title) {
    try {
      await pool.query(
